...
const normalUser = {phoneNumber: "+15555555555", isAdmin: true, isActive: true };
...
describe("Core Logic", () => {
    ...
    it("should set a message when called by and admin with SET MESSAGE", async () => {
        const message = { Body: "SET MESSAGE Hello world!", From: admin.phoneNumber };
    ...
    it("should add a subscriber when non-admin sends a code", async () => {
        ...
        const user = await phoneNumberModel.findByPhoneNumber({phoneNumber: normalUser.phoneNumber});
    ...
    it("should send a message to subscriber when SEND CODE is sent by admin", async () => {
        ...
        expect(sendStub.calledWithExactly(normalUser.phoneNumber, responses.DEFAULT_MESSAGE));
    });

    it("should send a message to subscriber when CUSTOM CODE is sent by admin", async () => {
        const message = { Body: "custom test Hello world!", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);
    ...
    it("should send a message to all when CUSTOM ALL is sent by admin", async () => {
        const message = { Body: "custom all Hello world!", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);
    ...
    it("should add an admin as a subscriber when they send a code", async () => {
        ...
        const adminUser = await phoneNumberModel.findByPhoneNumber({phoneNumber: admin.phoneNumber});
    ...
    it("should change a code when it receives CHANGE CODE from admin", async () => {
        const message = { Body: "change code test changedtest", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);
        ...
        expect(response).to.equal(responses.CHANGE_CODE.replace("%CODE1%", "TEST").replace("%CODE2%", "CHANGEDTEST"));
    ...
    it("should remove a code when it receives REMOVE CODE from admin", async () => {
        const message = { Body: "remove code changedtest", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);

        // Make sure it removed code
        const codes = await stateModel.getCampaignCodes();
        expect(!codes.includes("CHANGEDTEST"));
        expect(response).to.equal(responses.REMOVE_CODE.replace("%CODE%", "CHANGEDTEST"));
    ...
    it("should add an admin when it receives ADD ADMIN", async () => {
        const message = { Body: "add admin (100) 003-1337", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);

        const newAdmin = await phoneNumberModel.findByPhoneNumber({phoneNumber: "+11000031337"});
        expect(newAdmin).to.exist;
        expect(response).to.equal(responses.ADD_ADMIN.replace("%PHONE%", "(100)003-1337"));
    });

    it("should send error when ADD ADMIN can't parse phone", async () => {
        const message = { Body: "add admin (1X0) 0X3-1337", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);

        const newAdmin = await phoneNumberModel.findByPhoneNumber({phoneNumber: "+11000013117"});
        expect(newAdmin).to.not.exist;
        expect(response).to.equal(responses.FAILED_PARSE_PHONE.replace("%PHONE%","(1X0)0X3-1337"));
    });

    it("should remove an admin when it receives REMOVE ADMIN", async () => {
        const message = { Body: "remove admin (100)003-1337", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);

        const notAdmin = await phoneNumberModel.findByPhoneNumber({phoneNumber: "+11000031337"});
        expect(notAdmin).to.exist;
        expect(notAdmin.isAdmin).to.be.false;
        expect(response).to.equal(responses.REMOVE_ADMIN.replace("%PHONE%", "(100)003-1337"));
    });

    it("should send error when REMOVE ADMIN can't parse phone", async () => {
        const message = { Body: "remove admin (1X0)0X3-1337", From: admin.phoneNumber };
        const response = await logic.decipherMessage(message);
    ...
    it("should set normal user to inactive when they send STOP", async () => {
        ...
        const normal = await phoneNumberModel.findByPhoneNumber({phoneNumber: normalUser.phoneNumber});
    ...
    describe("admin sending unknown or incomplete commands", () => {
        const badCommands = ["sup doge", "NOTACODE", "add", "send","add admin", "remove admin", "add code", "remove code", "change code alice", "set message", "custom", "custom test"];
    ...
    describe("non-admin calling admin functions",() => {
        const validAdminCommands = ["send test1", "add admin (555)555-5555", "remove admin (234)567-8910", "add code bacon", "remove code test2", "change code test1 love", "set message Hello world!", "custom all Hello world!", "status", "shutdown"];
