...
const logic = {
  decipherMessage: async (reqBody) => {
    try {
      ...
      switch (message) {
        case "STOP":
          await logic.endSubscription(phone);
          return;
        case "UNSTOP":
        case "START":
          await logic.startSubscription(phone);
          return;
      }

      // Handle valid campaign code
      if (campaignCodes.includes(message)) {
        const subExists = await phoneNumberModel.findByPhoneNumber({phoneNumber: phone});
        ...
        await reportingModel.incrementCount({campaignCode: message, fieldName});
        return responses.VALID_CAMPAIGN_CODE;
      }
      ...
      if (phoneNumber && phoneNumber.isAdmin && phoneNumber.isActive) {
        if (message.split(" ").length > 1) {
          ...
          if (strCmd[0] === "SEND" && campaignCodes.includes(strCmd[1])) {
            ...
            const count = await logic.handleDeliveryMessage(strCmd[1]);
            ...
            const count = await logic.sendCustomMessage(strCmd[1], reqBody.Body);
  ...
  startSubscription: async (phoneNumber) => {
    await phoneNumberModel.createOrUpdate({
      phoneNumber,
      isActive: true,
    });
    const sub = await phoneNumberModel.findByPhoneNumber({phoneNumber});
    await reportingModel.incrementCount({campaignCode: sub.campaignCode, fieldName: "startSubscriptionCount"});
  },
  endSubscription: async (phoneNumber) => {
    const sub = await phoneNumberModel.findByPhoneNumber({phoneNumber});
    await phoneNumberModel.createOrUpdate({
      phoneNumber,
      isActive: false,
    });
    await reportingModel.incrementCount({campaignCode: sub.campaignCode, fieldName: "endSubscriptionCount"});
  },
  handleDeliveryMessage: async (campaignCode) => {
    ...
    for (let sub of subscribers) {
      const success = await messenger.send(sub.phoneNumber, message);
      await phoneNumberModel.incrementSendCount({ phoneNumber: sub.phoneNumber, success });
      await reportingModel.incrementCount({campaignCode, fieldName: success ? "sentCount" : "failedCount"});
    }
