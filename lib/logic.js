...
const logic = {
  decipherMessage: async (reqBody) => {
    try {
      ...
      // Allow admin to use subscriber workflow by prepending SUBSCRIBE to command
      let isSubscriberCmd = false;
      if (message.includes('SUBSCRIBE')) {
        isSubscriberCmd = true;
        message = message.replace('SUBSCRIBE ', '');
      }
  
      if (phoneNumber && phoneNumber.isAdmin && !isSubscriberCmd) {
  ...
  setDefaultMessage: async (unparsedMessage) => {
    // Remove first two commands from message
    const message = unparsedMessage.split(" ").splice(2).join(" ");
    const existing = stateModel.getSetting("deliveryMessage");
    if (typeof existing === "string") { // Could still be '' which is falsy so check for string type
  ...
  addAdmin: async (newAdmin) => {
    ...
    try {
      ...
      logger.error(
        `Could not parse '${newAdmin}' as a phone number: ${JSON.stringify(e.message)}`
      );
  ...
  removeAdmin: async (admin) => {
    ...
    try {
      ...
      logger.error(
        `Could not parse '${admin}' as a phone number: ${JSON.stringify(e.message)}`
      );
